...
int gFreeImage::LoadImage(char *filename)
{
	...
	{
	  // ok, let's load the file in following codes...
	}
	else return 0;
	...
	memcpy(FreeImage_GetBits(Bitmap), imageData, width * height * (24 / 8));

	unsigned char *imageDatatest = (unsigned char *)FreeImage_GetBits(Bitmap);
...
int gFreeImage::updateImageData(unsigned char *m_imageData, int m_width, int m_high,int BitsPP)
{
	int returnvalue = 0;

	bitmap = FreeImage_Allocate(m_width, m_high, BitsPP, 0, 0, 0);

	memcpy(FreeImage_GetBits(bitmap), m_imageData, m_width * m_high * (BitsPP / 8));
